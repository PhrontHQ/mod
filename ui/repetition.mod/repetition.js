    ...
    /** @lends Iteration.prototype # */ {
        ...
        injectIntoDocument: {
            value: function (index) {
                if (this._drawnIndex !== null) {
                    if (logger.isDebug) {
                        logger.debug(
                            "Iteration:%s retracting from index %s and injecting at %s",
                            Object.hash(this),
                            this._drawnIndex,
                            index
                        );
                ...
                if (this._childComponents.length > 0) {
                    for (var i = 0; i < this._childComponents.length; i++) {
                        ...
                        if (childComponent._completedFirstDraw === true) {
                            throw Error(
                                "Repetiton:%s child component %O has already drawn.",
                                Object.hash(this),
                                childComponent
                            );
                        }
        ...
        retractFromDocument: {
            value: function () {
                if (logger.isDebug) {
                    logger.debug(
                        "Iteration:%s retractFromDocument drawnIndex: %s",
                        Object.hash(this),
                        this._drawnIndex
                    );
                }
 ...
 * @extends Component
 */
exports.Repetition = class Repetition extends Component {
    /**
     * As dispatching has been implemented in key  setters, limit use of default method
     * for corresponding keys.
     * @private
    ...
    //     value: PropertyChanges.prototype.makePropertyObservable
    // },
    makePropertyObservable(key) {
        if (key === "selection") {
            this.isSelectionEnabled = true;
        }
        super.makePropertyObservable(key);
    }

    handleSelectionRangeChange(add, remove) {
        if (this.iterations) {
            var iterationsMap,
    ...
    handleVisibleIndexesRangeChange(plus, minus, index) {
        var plusContent, i;
    ...
    handleControllerOrganizedContentRangeChange(plus, minus, index) {
        ...
        if (this.__controllerOrganizedContent) {
            if (this._visibleIndexes) {
                if (this.organizedContent.length > this._visibleIndexes.length) {
                    ...
                    if (this._isListeningToOrganizedContentChanges) {
                        this.handleOrganizedContentRangeChange(
                            0,
                            this.organizedContent.length - this._visibleIndexes.length,
                            this._visibleIndexes.length
                        );
                    }
                }
     ...
     * @private
     */
    _handleContentControllerAllowsMultipleSelectionChange(value) {
     ...
     * @private
     */
    _handleSetupRequirementsChange(canSetUp) {
        if (canSetUp) {
     ...
     * @private
     */
    _handleInnerTemplateChange(innerTemplate) {
     ...
     * @private
     */
    cleanupDeletedComponentTree(permanently) {
    ...
    _leTagIterationTemplate(template) {
     ...
     * @private
     */
    _expandIterationTemplateParameters(template) {
        var owner = this,
            argumentsTemplate,
            collisionTable,
            externalLabels,
            ...
            if (owner._ownerDocumentPart) {
                ...
                for (var i = 0, label; (label = labels[i]); i++) {
                    ...
                    if (externalLabels.indexOf(newLabel) >= 0) {
                        ...
                        template.setObjectMetadata(newLabel, metadata.require, metadata.label, metadata.owner);
                    }
                }
            }
        }
    ...
    _createIteration() {
        ...
        this._iterationCreationPromise = this._iterationCreationPromise.then(function () {
            ...
            promise = self._iterationTemplate.instantiateWithInstances(instances, _document).then(function (part) {
     ...
     * @private
     */
    handleOrganizedContentRangeChange(plusLength, minusLength, index) {
        var start = index,
            freedIterations,
            freedIteration,
        ...
        if (logger.isDebug) {
            logger.debug(
                "Repetition:%s content changed +%s@%s %O -%s %O ",
                Object.hash(this),
                plusLength ? plusLength : 0,
                index,
                minusLength ? minusLength : 0
            );
            logger.debug(
                "Repetition:%s +%s -%s iterations",
                Object.hash(this),
                addIterationsCount,
                removeIterationsCount
            );
        }
        ...
        if (addIterationsCount > 0) {
            ...
            for (i = reusableIterationsCount, j = 0; i < plusLength; i++, j++) {
                ...
                if (logger.isDebug) {
                    logger.debug(
                        "Repetition:%s Iteration:%s object set to: %",
                        Object.hash(this),
    ...
    _addDirtyClassListIteration_forEach_callback(element, index, iteration) {
        var component;
    ...
    _addDirtyClassListIteration(iteration) {
    ...
    _draw_forEachIterationElement_callback(element, index, iteration) {
    ...
    _updateDrawnIndexes(drawnIndex) {
    ...
    handleIsSelectionEnabledChange(selectionTracking) {
    ...
    handlePress(event) {
        ...
        if (iteration && this._currentActiveIteration === iteration) {
            ...
            if (!iteration.selected) {
                ...
                if (this.selectedIndexes.length == this.contentController.maxSelectionLength) {
                    this._ignoreSelection();
                    return;
                } else {
     ...
     * @private
     */
    _findIterationContainingElement(element) {
    ...
    static {
        Component.defineProperties(Repetition.prototype, {
            /**
            ...
            allowsMultipleSelection: {
                ...
                get: function () {
                    return this.contentController !== void 0 && this.contentController !== null
                        ? this.contentController.allowsMultipleSelection
                        : false;
                },
            },
            ...
            _controllerOrganizedContent: {
                ...
                set: function (value) {
                    if (this.__controllerOrganizedContent !== value) {
                        if (
                            this.__controllerOrganizedContent &&
                            this.__controllerOrganizedContent.removeRangeChangeListener
                        ) {
                            this.__controllerOrganizedContent.removeRangeChangeListener(
                                this,
                                "controllerOrganizedContent"
                            );
                        }
                        this.__controllerOrganizedContent = value;
                        if (
                            this.__controllerOrganizedContent &&
                            this.__controllerOrganizedContent.addRangeChangeListener
                        ) {
                            this.__controllerOrganizedContent.addRangeChangeListener(
                                this,
                                "controllerOrganizedContent"
