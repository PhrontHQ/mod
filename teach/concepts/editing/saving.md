...
Since the introduction of instance change tracking that allow to saveChanges a
...
#### New Design
1. Move change tracking from `DataService` to `EditingContext`
2. `EditingContext` would receive an object or objects to track. These can be created by anywhere -- Components, RawDataServices, etc. 
...
4. `DataService` creates and manages a default `EditingContext` that will track changes for any objects that are not handled by another EditingContext
