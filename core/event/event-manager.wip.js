...
var EventManager = exports.EventManager = Montage.specialize(/** @lends EventManager.prototype # */ {
   ...
   registerTargetEventListener: {
    ...
    /**
     ...
     - If handleEvent() know the dispatch is sync, we can re-use the same handleEvent() and keep track of the stack
     - Otherwise, coukd we do: 

     function EventManager_handleEvent(event) {
        - Let currentHandleEvent = this.handleEvent,
        - nestedHandleEvent = (nestedEvent) => {

            let referrerEvent = this._eventByNestedHandleEvent.get(nestedHandleEvent);
            nestedEvent.referrer = referrerEvent;
            let promise = EventManager_handleEvent.call(this, event);
            if(promise) {
                promise.then(() => {
                    this._eventByNestedHandleEvent.delete(nestedHandleEvent);
                })
            }
            ...
        }
        this._eventByNestedHandleEvent.set(nestedHandleEvent, event);
    ...
    handleEvent: {
        ...
        value: function EventManager_handleEvent(event) {
            ...
            eventPath = this.composedPathForEvent(mutableEvent);
            /*
                This isn't used anywhere and could override ._composedPath set from the outside
                The only use in SynchronizationService - readOperation.composedPath().delete(this.destinationDataService);
                should we do:
                  if(!mutableEvent._composedPath) {
    ...
    /**
     * Introduced to give the ability to the context creating and dispatching an event to also
     *
     * Benoit todo: Make this more generic and configurable to be applicable to different trees.
     *
     * @function
    ...
    composedPathForEvent: {
        ...
        value: function (event) {
            /* 
                if event is a MutableEvent, which it should be,
                because currently MutableEvent doesn't wrap composedPath() to return :
                    return this._composedPath || this._event.composedPath()

                for a DOM event, 

                event.composedPath() and this.eventPathForTarget(event.target) returns the same thing with one exception:

                the array returned by our this.eventPathForTarget(event.target) contains this.application as the last object
                where the DOM obviously doesn't

                Now that a native composedPath() exists it should be much faster to use it than re-compute the same thing,
                But we have to change how we handle application.

                We should be able to handler it as a one-off before the loop for capture phase
                and as a one-off after the loop for bubble phase.

                To be tested but that should work

                Also to do fix: 

                    this._eventPathForTarget() returns an array that ends with [..., Application, ApplicationDelegate]

            */
            return event.composedPath() || this.eventPathForTarget(event.target);
        }
    },
